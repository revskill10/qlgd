  ...
  def generate    
    if self.lop_mon_hoc
      ...
      if sch.count > 0
        sch.each do |s|
          begin
            lich = self.lop_mon_hoc.lich_trinh_giang_days.with_giang_vien(self.giang_vien.id).normal.create(thoi_gian: s.to_time, so_tiet: self.so_tiet, phong: self.phong)
            lich.accept!          
          rescue Exception => e
            puts e
          end
