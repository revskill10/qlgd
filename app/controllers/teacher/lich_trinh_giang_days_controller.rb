class Teacher::LichTrinhGiangDaysController < ApplicationController
	...
	def create_bosung
		...
		@lich = @lop.lich_trinh_giang_days.with_giang_vien(gv.id).bosung.where(thoi_gian: thoi_gian).first
		if @lich.nil?	
			@lich = @lop.lich_trinh_giang_days.with_giang_vien(gv.id).bosung.create(thoi_gian: thoi_gian,phong: params[:phong], so_tiet: params[:so_tiet].to_i, thuc_hanh: params[:thuc_hanh])			
		end
		@lichs = @lop.lich_trinh_giang_days.with_giang_vien(gv.id).bosung.map { |l| LopLichTrinhGiangDaySerializer.new(l)}
		render json: @lichs, :root => false
	end
	def update_bosung
		@lop = LopMonHoc.find(params[:lop_id])	
		authorize @lop, update?	
		gv = current_user.imageable
		@lich = @lop.lich_trinh_giang_days.with_giang_vien(gv.id).bosung.find(params[:id])
		if @lich
			hour = LichTrinhGiangDay::TIET2[params[:tiet_bat_dau].to_i][0].to_s
			minute = LichTrinhGiangDay::TIET2[params[:tiet_bat_dau].to_i][1].to_s
			thoi_gian = Time.strptime(hour + ":" + minute + " " +params[:thoi_gian], "%H:%M %d/%m/%Y")
	...
	def nghiday
		...
		authorize @lop, update?
		gv = current_user.imageable
		@lich = @lop.lich_trinh_giang_days.with_giang_vien(gv.id).find(params[:id])
		if @lich and @lich.can_nghiday?
			@lich.note = params[:note]
			@lich.nghiday!
			@lich.save!
		end
		@lichs = @lop.lich_trinh_giang_days.with_giang_vien(gv.id).map { |l| LopLichTrinhGiangDaySerializer.new(l)}
		render json: @lichs, :root => false
	end
	def unnghiday
		@lop = LopMonHoc.find(params[:lop_id])
	...
	def getcontent
		...
		#authorize @lop, update?
		gv = current_user.imageable		
		@lichs = @lop.lich_trinh_giang_days.normal_or_bosung.accepted.with_giang_vien(gv.id).map { |l| LichTrinhGiangDaySerializer.new(l.decorate)}
		render json: @lichs, :root => false
	end
	def content
		@lop = LopMonHoc.find(params[:lop_id])
