LichTrinhGiangDayPolicy = Struct.new(:user, :lich_trinh_giang_day) do
  ...
  def update?
    ...
    !(lich_trinh_giang_day.state == "nghile") and !(lich_trinh_giang_day.state == "nghiday") and lich_trinh_giang_day.accepted? and user.get_lichs.map(&:id).include?(lich_trinh_giang_day.id) and !user.giang_vien(lich_trinh_giang_day.lop_mon_hoc).nil?
  end
