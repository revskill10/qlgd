...
describe LichTrinhGiangDay do  
  ...
  it "should require giang_vien, thoi gian va so tiet" do 
    ...
    gv = FactoryGirl.create(:giang_vien)
  	lich = lop.lich_trinh_giang_days.build(:so_tiet => 2, :thoi_gian => Time.new(2013, 8, 12, 6, 30))
    #lich.giang_vien = gv
  ...
  it "should require giang vien" do 
    lop = FactoryGirl.create(:lop_mon_hoc)    
    lich = lop.lich_trinh_giang_days.build(:so_tiet => 2, :thoi_gian => Time.new(2013, 8, 12, 6, 30), :giang_vien_id => nil)
    lich.valid?.should be_false
  end
  ...
  it "should not be duplicated" do 
    ...
    lop.lich_trinh_giang_days.where("thoi_gian = TIMESTAMP ?", lich.thoi_gian.to_time.strftime("%Y-%m-%d %H:%M:00")).first.should_not be_nil
  	lich2 = lop.lich_trinh_giang_days.create(:so_tiet => 2, :thoi_gian => Time.new(2013, 8, 12, 6, 30))
    lich2.giang_vien = gv
    #lich2.save!
  	lich2.errors[:name].should  include('duplicates thoi_gian')
    lich3 = lop.lich_trinh_giang_days.where(:so_tiet => 2, :thoi_gian => Time.new(2013, 8, 12, 6, 30)).first_or_create!    
    lich3.should == lich    
  end
